...
export class AutoRelater {
  ...
  private addRelation(
    ...
    const isOne =
      (spec.isPrimaryKey &&
        !_.some(fkFields, (f) => f.isPrimaryKey && f.source_column !== fkFieldName)) ||
      (!!spec.isUnique &&
        !_.some(
          fkFields,
          (f) => f.isUnique === spec.isUnique && f.source_column !== fkFieldName,
        ));
    ...
    if (spec.isPrimaryKey) {
      ...
      const otherKeys = _.filter(
        fkFields,
        (k) => k.isForeignKey && k.isPrimaryKey && k.source_column !== fkFieldName,
      );
