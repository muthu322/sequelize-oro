...
import { AutoOptions, CaseFileOption, CaseOption, Field, IndexSpec, LangOption, makeIndent, makeTableName, pluralize, qNameJoin, qNameSplit, recase, Relation, singularize, TableData, TSField, replace } from "./types";
    ...
    this.relations.forEach((rel) => {
      ...
      if (!rel.isM2M) {
        if (rel.childTable === table) {
          ...
          if (!/_x_/i.test(rel.target_table)) {
            ...
            if (tableName && tableName !== "") {
              ...
              str += `${space[3]}as: '${replace(
                recase("c", rel.source_column),
                {
                  Id: "",
                  id: "",
                }
          ...
          if (rel.isOne) {
            ...
            const sing = _.upperFirst(
              singularize(rel.childProp)
            );
        ...
        if (rel.parentTable === table) {
          ...
          const otherModelPlural =
            _.upperFirst(lotherModelPlural);
    ...
    if (this.junction[table]) {
      this.junction[table].forEach((rel:any) => {
        ...
        str += `${space[3]}as: '${recase(
          "c",
          pluralize(
           replace(rel.source_model, { Id: "", id: "" })
          )
        )}',\n`;
        str += `${space[2]}});\n`;
      });
