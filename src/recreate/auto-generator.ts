...
export class AutoGenerator {
  ...
  private addMigrationTable(table: string) {
    ...
    const new_table_name = tableNameOrig + '_recreate_temp';
    str += `${space[3]}await queryInterface.renameTable('${tableNameOrig}', '${new_table_name}', { transaction });\n`;
    ...
    str += `${space[3]}const tableValues = [\n`;
    fields.forEach((field) => {
      str += `${space[4]}{\n`;
      str += `${space[5]}columnName: '${field}',\n`;
      str += `${space[5]}value: '${field}',\n`;
      str += `${space[4]}},\n`;
    });
    str += `${space[3]}];\n`;
