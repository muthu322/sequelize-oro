...
export class AutoGenerator {
    ...
    fieldAttrs.forEach((attr) => {
      ...
      if (attr === 'foreignKey') {
        if (foreignKey && foreignKey.isForeignKey) {
          ...
          str +=
            space[4] + "model: '" + fieldObj[attr].foreignSources.target_table + "',\n";
          str +=
            space[4] + "key: '" + fieldObj[attr].foreignSources.target_column + "',\n";
          str += space[3] + '}';
        } else {
          return true;
        }
  ...
  private getSqType(fieldObj: Field, attr: string): string {
    ...
    if (
      ...
      // integer subtypes
      val =
        'DataTypes.' + (typematch[0] === 'int' ? 'INTEGER' : typematch[0].toUpperCase());
